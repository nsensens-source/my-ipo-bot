...
# หุ้นในนี้จะทะลุตัวกรองทั้งหมด และถูกแยกแสดงในกระดานพิเศษให้เสมอ
...
def send_to_discord(df, title, webhook_url, history_header="D-1 to D-10 Trend"):
    ...
    header = f"🎯 **{title}** 🎯\n"
    cb = "```" 
    table_header = f"{cb}text\n{'Ticker':<6} | {'Price':<7} | {'Today':<8} | {history_header}\n"
    table_header += "-" * 105 + "\n"
    ...
    for i in range(0, len(tickers), chunk_size):
        ...
        for ticker in chunk:
            try:
                ...
                # ดึง % ย้อนหลัง 10 วัน (D-1 ถึง D-10) ด้วยวันที่ล็อกไว้เป๊ะๆ
                history_pcts = []
                valid_history = True
                for j in range(2, 12):
