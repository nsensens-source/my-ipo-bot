...
DISCORD_WEBHOOK_URL = os.getenv(
    'DISCORD_WEBHOOK_TOPMOVER', 
    'https://discord.com/api/webhooks/1484400920736563382/bHJtqjnNCFRMXGgdz6kIE3lGFCfx4durGe5R5RGPpM12ad9yfkyN0LQ0XK_nira4GML2'
)

# --- ตั้งค่าตัวกรองหุ้น (Filters) ---
TOP_N = 100                     # จำนวนหุ้น Top Mover ที่ต้องการจัดอันดับ
...
# ใส่ชื่อหุ้นที่ต้องการลงในวงเล็บนี้ได้เลย หุ้นกลุ่มนี้จะทะลุตัวกรองและถูกแสดงผลเสมอ!
...
def send_to_discord(df, title, webhook_url, history_header="D-1 to D-10 Trend"):
    ...
    header = f"🎯 **{title}** 🎯\n"
    cb = "```" 
    table_header = f"{cb}text\n{'Ticker':<6} | {'Price':<7} | {'Today':<8} | {'10D Sum':<17} | {history_header}\n"
    table_header += "-" * 125 + "\n"
    ...
    for _, row in df.iterrows():
        # เพิ่มคอลัมน์ Sum10D ตรงกลางก่อนหน้า History
        line = f"{row['Ticker']:<6} | {row['Price']:>7.2f} | {row['Today']:<8} | {row['Sum10D']:<17} | {row['History']}\n"
...
def main():
    ...
    for i in range(0, len(tickers), chunk_size):
        ...
        # โหลดข้อมูลย้อนหลัง 20 วัน เพื่อให้ได้วันทำการครบ 12 วัน (สำหรับคำนวณ % 10 วัน)
        ...
        for ticker in chunk:
            try:
                ...
                # ดึงราคาจากวันที่ถูกล็อกไว้เท่านั้น (d-1 คือล่าสุด, d-2 คือเมื่อวาน)
                ...
                # คำนวณ Volume เฉลี่ย 5 วันล่าสุด
                ...
                # --- 🎯 คำนวณ สรุป 10 วันหลังสุด (จำนวนวันที่ ขึ้น/ลง และ % รวม) ---
                up_days = 0
                down_days = 0
                for j in range(1, 11): 
                ...
                results.append({
                    ...
                    'SortVal': today_pct_val,             # เก็บค่าเดิมไว้สำหรับคำนวณ Sector 
                    'AbsSortVal': abs(today_pct_val),     # เพิ่มค่าสัมบูรณ์สำหรับการจัดอันดับ Top Mover
                    'Sum10D': sum10d_str,
    ...
    # 1. จัดอันดับ Top N (เรียงจาก % การขยับที่แรงที่สุด ไม่ว่าจะบวกหรือลบ)
    top_movers = all_df.sort_values(by='AbsSortVal', ascending=False).head(TOP_N)
    
    # 2. แยกกระดาน Watchlist 
    watchlist_df = all_df[all_df['IsWatchlist']].sort_values(by='AbsSortVal', ascending=False)
    ...
    for ticker in top_movers['Ticker']:
        sector = 'Unknown'
    ...
    if not valid_sectors.empty:
        # ใช้ SortVal ธรรมดาในการหาค่าเฉลี่ย เพื่อให้ดูว่า Sector นั้นโดยรวมบวกหรือลบ
    ...
    unknown_count = top_movers[top_movers['Sector'] == 'Unknown'].shape[0]
    if unknown_count > 0:
        sector_msg_content += f"\n🔸 **ไม่สามารถระบุกลุ่มได้ (Unknown / อื่นๆ)**: {unknown_count} ตัว\n"

    # --- ส่งข้อความเข้า Discord ---
    print(f"ส่งข้อมูล Top {TOP_N} Movers...")
    title_top = f"TOP {TOP_N} MOVERS (Price > ${MIN_PRICE}, Avg Vol > ${MIN_DOLLAR_VOLUME/1_000_000}M)"
    send_to_discord(top_movers, title_top, DISCORD_WEBHOOK_URL, history_header="D-1 to D-10 Trend")
